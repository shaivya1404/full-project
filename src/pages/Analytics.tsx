...
export const AnalyticsPage = () => {
  ...
  useEffect(() => {
    ...
    const baseUrl = import.meta.env.VITE_API_BASE_URL || 'https://backend-v2wh.onrender.com/api';
  ...
  const { data: botStats, isLoading: isBotLoading } = useBotAnalytics(dateRange);
  const { data: unansweredData, isLoading: isUnansweredLoading } = useUnansweredQuestions(
    unansweredPage,
    unansweredLimit,
  ...
  const handleExportUnanswered = async () => {
    try {
      ...
      link.href = url;
      link.setAttribute('download', `unanswered-questions-${dateRange.startDate}-to-${dateRange.endDate}.csv`);
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              <div className="lg:col-span-2">
                <div className="flex items-center justify-between mb-4">
                  <h2 className="text-xl font-bold text-gray-900 dark:text-white">Unanswered Questions</h2>
                </div>
                <UnansweredQuestionsTable
              ...
              <div className="space-y-6">
                <KnowledgeGapsCard gaps={botStats?.knowledgeGaps || []} isLoading={isBotLoading} />
                <BotImprovementRecommendations insights={botStats?.improvementInsights || []} isLoading={isBotLoading} />
              </div>
            </div>
